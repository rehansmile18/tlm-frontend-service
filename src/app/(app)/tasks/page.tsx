...
export default function TasksPage() {
  ...
  return (
    <>
                ...
                {filteredItems.map((task) => (
                  <TableRow key={task._id} className="cursor-pointer" onClick={() => router.push(`/tasks/${task._id}`)}>
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <input
                        type="checkbox"
                        aria-label={t("common.selectRow", { name: task.name })}
                        className="size-4 rounded border-input accent-primary"
                        checked={selected.includes(task._id)}
                        onChange={() =>
                          setSelected((prev) =>
                            prev.includes(task._id) ? prev.filter((s) => s !== task._id) : [...prev, task._id]
                          )
                        }
                      />
                    </TableCell>
      ...
      <BulkArchiveBar
        targets={selected
          .map((id) => filteredItems.find((x) => x._id === id))
          .filter((r) => r !== undefined)
          .map((x) => ({ id: x._id, name: x.name }))}
        onArchive={(id) => tasksApi.archive(id)}
        invalidateKeys={["tasks"]}
        onClear={() => setSelected([])}
      />
